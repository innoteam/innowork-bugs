    ...
    public function getWidgetXml()
    {
        ...
        if ($search_result_count > 0) {
            ...
            foreach ($search_result as $bug) {
                $xml .= '
                    <link row="'.$row.'" col="0" halign="left" valign="top">
                        <args>
                            ...
                            .WuiXml::cdata(
                                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                    'innoworkbugs', array(array('view', 'showbug', array('id' => $bug['id'])))
                                )
                            ).'</link>
                	    </args>
                	</link>
        ...
        $xml .= '
            ...
            <args>
                ...
                .WuiXml::cdata(
                    \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                        'innoworkbugs', array(array('view', 'newbug', array()))
                    )
                ).'</action>
            </args>
            </button>';
            ...
            $xml .= '<button>
                <args>
                    ...
                    .WuiXml::cdata(
                        \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                            'innoworkbugs', 
                            array(array('view', 'default', array('filter' => 'true', 'filter_assignedto' => $container->getCurrentUser()->getUserId())))
                        )
                    ).'</action>
                </args>
                </button>';
