...
class InnoworkbugsPanelViews extends \Innomatic\Desktop\Panel\PanelViews
{
                ...
                'page', array(
                    ...
                    'toolbars' => array(
                        new WuiInnomaticToolbar(
                            'view', array('toolbars' => $this->toolbars, 'toolbar' => 'true')
                        ),
                        new WuiInnomaticToolBar(
                            'core', array('toolbars' => $this->innoworkCore->getMainToolBar(), 'toolbar' => 'true')
                        )
                    ),
    ...
    public function viewDefault($eventData)
    {
        ...
        if (isset($eventData['filter'])) {
            ...
            $source_filter_sk = new WuiSessionKey(
                'source_filter',
                array('value' => $eventData['filter_sourceid'])
            );
            ...
            $type_filter_sk = new WuiSessionKey(
                'type_filter', array('value' => $eventData['filter_severityid'])
            );
            ...
            $resolution_filter_sk = new WuiSessionKey(
                'resolution_filter',
                array('value' => $eventData['filter_resolutionid'])
            );
            if ($eventData['filter_resolutionid'] != 0) {
                $search_keys['resolutionid'] = $eventData['filter_resolutionid'];
            }
            ...
            if (strlen($source_filter_sk->mValue) and $source_filter_sk->mValue != 0) {
        ...
        $tab_sess = new WuiSessionKey(
            'innoworkbugstab', array('value' => $eventData['done'])
        );
        ...
        $table = new WuiTable(
            'bugs_done_'.$eventData['done'], array(
        ...
        $headers[8]['link'] = \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
            '', array(array('view', 'default', array('sortby' => '9')))
        );
            ...
            <vertgroup>
              <children>
                ...
                <label><name>title</name>
                  <args>
                    <bold>true</bold>
                    <label>'.($this->localeCatalog->getStr((isset($eventData['done'])and $eventData['done'] == 'true') ? 'donebugs.label' : 'bugs.label')).'</label>
                  </args>
                </label>
                          ...
                          <children>
                            <link>
                              <args>
                                <label>'
                                .WuiXml::cdata(
                                    $bug['id'].' - '.$country->FormatShortArrayDate(
                                ...
                                .WuiXml::cdata(
                                    \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                        '',
                                        array(
                                            array('view', 'showbug', array('id' => $bug['id']))
                                        )
                                    )
                                ).'</link>
                                </args>
                                ...
                                array(
                                    'view' => array(
                                        'show' => array(
                                            ...
                                            'horiz' => 'true',
                                            'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                                '', array(array('view', 'showbug', array('id' => $bug['id'])))
                                            )
                                        ),
                                        'done' => array(
                                            ...
                                            'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                                ...
                                                array(
                                                    array('view', 'default', ''),
                                                    array('action', 'editbug', array('id' => $bug['id'], 'done' => $done_action))
                                                )
                                            )
                                        ),
                                        'trash' => array(
                                            ...
                                            'action' => \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                                ...
                                                array(
                                                    array('view', 'default', ''),
                                                    array('action', 'trashbug', array('id' => $bug['id']))
                                                )
                                            )
                                        )
                                    )
    ...
    public function viewNewbug($eventData)
    {
        // Companies
                      ...
                      <children>
                        <grid>
                          ...
                          <args>
                                ...
                                .WuiXml::cdata(
                                    WuiEventsCall::buildEventsCallString('', array(array('view', 'searchproject')))
                                ).'</autocompletesearchurl>
                            <disp>action</disp>
                            <size>30</size>
                          </args>
    ...
    public function viewShowbug($eventData)
    {
        ...
        $bug_data = $innowork_bug->getItem($this->innomaticContainer->getCurrentUser()->getUserId());
        ...
        if ($bug_data['done'] == $this->innomaticContainer->getCurrentDomain()->getDataAccess()->fmttrue) {
            $done_icon = 'misc3';
            $done_action = 'false';
            $done_label = 'setundone.button';
        } else {
            ...
            <horizgroup>
                  ...
                  <children>
                      ...
                      <args>
                            ...
                            .WuiXml::cdata(
                                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                    '',
                                    array(
                                        array('view', 'showbug', array('id' => $eventData['id'])),
                                        array('action', 'editbug', array('id' => $eventData['id']))
                                    )
                                )
                            ).'</action>
                      </args>
                      <children>
                        ...
                        <combobox><name>assignedto</name>
                          <args>
                            ...
                            <label><args><label>'.$this->localeCatalog->getStr('solution.label').'</label></args></label>
                      ...
                      <children>
                        ...
                        <button>
                          <args>
                            ...
                            .WuiXml::cdata(
                                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                    '', array(array('view', 'addmessage', array('bugid' => $eventData['id'])))
                                )
                            ).'</action>
                          </args>
                        </button>
                      ...
                      <args>
                        ...
                        .WuiXml::cdata(
                            \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                ...
                                array(
                                    array('view', 'default', ''),
                                    array('action', 'editbug', array('id' => $eventData['id'], 'done' => $done_action))
                                )
                            )
                        ).'</action>
                          ...
                          <args>
                            ...
                            .WuiXml::cdata(
                                \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                    ...
                                    array(
                                        array('view', 'default'),
                                        array('action', 'trashbug', array('id' => $eventData['id']))
                                    )
                                )
                            ).'</action>
                          </args>
                    ...
                    <iframe row="2" col="0"><name>messages</name>
                      <args>
                        ...
                        .WuiXml::cdata(
                            \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                '', array(array('view', 'bugmessages', array('bugid' => $eventData['id'])))
                            )
                        ).'</source>
              ...
              <innoworkitemacl><name>itemacl</name>
                <args>
                    ...
                    .WuiXml::cdata(
                        \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                            '', array(array('view', 'showbug', array('id' => $eventData['id'])))
                        )
                    ).'</defaultaction>
                </args>
              </innoworkitemacl>
    ...
    public function viewBugmessages($eventData)
    {
        $innowork_bug = new InnoworkBug(
            $this->innomaticContainer->getDataAccess(),
            $this->innomaticContainer->getCurrentDomain()->getDataAccess(),
            ...
            $this->xml .= '
                <vertgroup row="'.$row.'" col="0" halign="" valign="top">
                  ...
                  <children>
                    <label>
                      <args>
                        <label>'.WuiXml::cdata($country->FormatShortArrayDate($message['creationdate'])).'</label>
                        <compact>true</compact>
                      </args>
                    </label>
                    <label>
                      <args>
                        <label>'.WuiXml::cdata($country->FormatArrayTime($message['creationdate'])).'</label>
                        <compact>true</compact>
                      </args>
                    </label>
                    <label>
                      ...
                      <action>'
                        .WuiXml::cdata(
                            \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                                '',
                                array(
                                    array('view', 'bugmessages', array('bugid' => $eventData['bugid'])),
                                    array(
                                        'action',
                                        'removemessage',
                                        array(
                  ...
                  <args>
                    ...
                    .WuiXml::cdata(
                        \Innomatic\Wui\Dispatch\WuiEventsCall::buildEventsCallString(
                            ...
                            array(
                                array('view', 'bugmessages', array('bugid' => $eventData['bugid'])),
                                array('action', 'newmessage', array('bugid' => $eventData['bugid']))
                            )
                        )
                    ).'</action>
                  </args>
